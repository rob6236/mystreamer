...
export default function MyVideosGrid({ uid }: { uid: string | null }) {
  ...
  React.useEffect(() => {
    async function run() {
      ...
      try {
        ...
        // Hide placeholder thumbs, dedupe, newest first already applied
  ...
  if (!uid) {
    return (
      <p className="text-sm">
        Please <Link href="/home" className="underline">sign in</Link> to view your uploads.
      </p>
    );
  }
  ...
  if (rows.length === 0) {
    return (
      <p className="text-sm">
        You haven’t uploaded any videos yet. Use the upload form above to add your first video.
      </p>
    );
  }
